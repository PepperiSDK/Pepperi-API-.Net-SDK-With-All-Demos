...
namespace Pepperi.SDK.Endpoints.Base
{
    public class BaseEndpoint_For_UserDefinedCollections
    {
        ...
        #region Bulk File Upload

        public UDC_UploadFile_Result BulkUploadFile(string schemeName, string filePath, bool? overwriteObject = null, bool? overwriteTable = null, int poolingInternvalInMs = 5000, int numberOfPoolingAttempts = 500, string multiFilesOverwriteKey = null)
        {
            var fileExtention = GetValidatedFileExtention(filePath);
            var fileData = ReadFileByteArray(filePath);

            return BulkUploadFile(schemeName, fileData, fileExtention, overwriteObject, overwriteTable, poolingInternvalInMs, numberOfPoolingAttempts, multiFilesOverwriteKey);
        }

        public UDC_UploadFile_Result BulkUploadViaFileApi<TData>(string schemeName, IEnumerable<TData> data, bool? overwriteObject = null, bool? overwriteTable = null, int poolingInternvalInMs = 5000, int numberOfPoolingAttempts = 500, string multiFilesOverwriteKey = null)
        {
            var jsonData = PepperiJsonSerializer.Serialize(data);
            return BulkUploadViaFileApi(schemeName, jsonData, overwriteObject, overwriteTable, poolingInternvalInMs, numberOfPoolingAttempts, multiFilesOverwriteKey);
        }

        public UDC_UploadFile_Result BulkUploadViaFileApi(string schemeName, string jsonData, bool? overwriteObject = null, bool? overwriteTable = null, int poolingInternvalInMs = 5000, int numberOfPoolingAttempts = 500, string multiFilesOverwriteKey = null)
        {
            var byteData = Encoding.UTF8.GetBytes(jsonData);
            return BulkUploadFile(schemeName, byteData, ".json", overwriteObject, overwriteTable, poolingInternvalInMs, numberOfPoolingAttempts, multiFilesOverwriteKey);
        }

        public UDC_UploadFile_Result BulkUploadFile(string schemeName, byte[] data, string fileExtention, bool? overwriteObject = null, bool? overwriteTable = null, 
            int poolingInternvalInMs = 5000, int numberOfPoolingAttempts = 500, string multiFilesOverwriteKey = null)
        {
            ValuesValidator.Validate(schemeName, "Scheme Name is empty!");
            ...
            var importFileResponse = ImportFileToPepperi(schemeName, ipaasUrlToGetFile, overwriteObject, overwriteTable, poolingInternvalInMs, numberOfPoolingAttempts, multiFilesOverwriteKey);
        ...
        #region Bulk Miltipart Import

        public string StartMultiFilesOverwrite(string schemeName)
        {
            var RequestUri = $"resources/{HttpUtility.UrlEncode(schemeName)}/import/multipart_overwrite/initialize";
            PepperiHttpClient PepperiHttpClient = new PepperiHttpClient(this.AuthentificationManager.IdpAuth, this.Logger);

            string postBody = PepperiJsonSerializer.Serialize(new {
            ...
            var parsedResponseBody = PepperiJsonSerializer.DeserializeOne<MultipartOverwriteInitializeResponse>(PepperiHttpClientResponse.Body);
            ValuesValidator.Validate(parsedResponseBody?.MultipartOverwriteKey, "Can't get Multipart Overwrite Key!");

            return parsedResponseBody?.MultipartOverwriteKey;
        }

        public UDC_MultiFilesOverwriteFinish_Result FinishMultiFilesOverwrite(string schemeName, string multiFilesOverwriteKey, int poolingInternvalInMs = 5000, int numberOfPoolingAttempts = 500)
        {
            ValuesValidator.Validate(multiFilesOverwriteKey, "Multipart Overwrite Key is empty!");
            var requestResult = SendFinishMultiFilesOverwrite(schemeName, multiFilesOverwriteKey);
            var auditLogId = requestResult.ExecutionUUID;
            var finalAuditLog = this.AuditLogs.AuditLogPolling(auditLogId, poolingInternvalInMs, numberOfPoolingAttempts);

            this.AuditLogs.ValidateSuccessAuditLog(finalAuditLog, "Finish Multi Files Overwrite");

            var resultObjectString = finalAuditLog?.AuditInfo?.ResultObject;
            ValuesValidator.Validate(resultObjectString, "Result Object is null!");
            
            var resultObject = PepperiJsonSerializer.DeserializeOne<UDC_MultiFilesOverwriteFinish_Result>(resultObjectString);
        ...
        private PepperiAuditLog ImportFileToPepperi(string schemeName, string fileUrl, bool? overwriteObject = null, bool? overwriteTable = null, int poolingInternvalInMs = 5000, int numberOfPoolingAttempts = 500, string multipartOverwriteKey = null)
        {
            var importFileResponse = SendImportFileToPepperiRequest(schemeName, fileUrl, overwriteObject, overwriteTable, multipartOverwriteKey);
        ...
        private PepperiResponseForAuditLog SendImportFileToPepperiRequest(string schemeName, string fileUrl, bool? overwriteObject = null, bool? overwriteTable = null, string multipartOverwriteKey = null)
        {
            var RequestUri = $"resources/{HttpUtility.UrlEncode(schemeName)}/import/file";
            PepperiHttpClient PepperiHttpClient = new PepperiHttpClient(this.AuthentificationManager.IdpAuth, this.Logger);

            var requestObject = new UDC_UploadFile_Request()
            {
                URI = fileUrl,
                OverwriteObject = overwriteObject,
                OverwriteTable = overwriteTable,
                MultipartOverwriteKey = multipartOverwriteKey
            };
            string postBody = PepperiJsonSerializer.Serialize(requestObject);
            string contentType = "application/json";
            string accept = "application/json";
        ...
        private PepperiResponseForAuditLog SendFinishMultiFilesOverwrite(string schemeName, string multiFilesOverwriteKey)
        {
            ValuesValidator.Validate(multiFilesOverwriteKey, "Multipart Overwrite Key is empty!");
            ValuesValidator.Validate(schemeName, "Scheme Name is empty!");
            var RequestUri = $"resources/{HttpUtility.UrlEncode(schemeName)}/import/multipart_overwrite/finalize";
            PepperiHttpClient PepperiHttpClient = new PepperiHttpClient(this.AuthentificationManager.IdpAuth, this.Logger);

            string postBody = PepperiJsonSerializer.Serialize(new
            {
                Key = multiFilesOverwriteKey
            });
            string contentType = "application/json";
            string accept = "application/json";
